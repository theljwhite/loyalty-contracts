...
//TODO *TO-TEST 12/20:
//Escrow state still needs more testing in a dedicated test file (test state switching)...
//...as loyalty program completing or all funds being paid has not been tested yet to ensure that...
//...escrow state will change to Completed

//other edge cases for the rewardConditions can prob be discovered and tested
//testing withdrawals more & withdrawal security
//especially owner withdrawals when escrow state is completed

//TODO *IN-General:
//Clean up un-needed variables (for example, does Escrow even need to be a struct?)
//do I need locked/paid booleans? IF so, do they need implemented?
//handling for emergencies/frozen and canceled loyalty program escrow state
...
contract LoyaltyERC1155Escrow is ERC1155Holder, Ownable {
  ...
  error OnlyLoyaltyTeamCanSetDepositKey();
  error ExceededMaxTokenIdsAmount();
  error OnlyLoyaltyProgramCanCall();
  error OnlyCreatorCanCall();
  error OnlyCreatorCanModifyEscrowSettings();
  error RewardTypeMustBeERC1155();
  ...
  error TokenIdsAndObjectivesLengthMismatch();
  error TokenIdsAndTiersLengthMismatch();
  error TiersMustBeActiveToUseTiersRewardCondition();
  error TokenIdsAndPayoutsFirstIndexesMustBeZeros();
  ...
  error MustChooseValidObjectiveIndexAsRewardGoal();
  error MustChooseValidTierIndexAsRewardGoal();
  error NotInIssuance();
  ...
  function onERC1155Received(
    ...
    emit ERC1155TokenReceived(_msgSender(), _from, _tokenId, block.timestamp);
  ...
  function onERC1155BatchReceived(
    ...
    emit ERC1155BatchReceived(_msgSender(), _from, _tokenIds, block.timestamp);
  ...
  function parseTokensAddToEscrow(
    ...
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      if (tokenBalances[_tokenIds[i]] != 0) {
        ...
        escrow.tokens.push(Token({ id: _tokenIds[i], value: _values[i] }));
  ...
  function unlockRewardsByObjective(address _user, uint256 _objIndex) private {
    ...
    if (
      tokenBalances[payout.tokenId] >= payout.amount && !rewardAlreadyRewarded
    ) {
      if (payout.amount > 0) {
        user.rewardGoalRewarded[_objIndex] = true;
        user.rewardedTokenBalances.push(
          UserTokenBalance({ tokenId: payout.tokenId, amount: payout.amount })
        );
    ...
    if (escrow.rewardCondition == RewardCondition.EachTier) {
      for (uint256 i = 0; i < _passedTiers.length; i++) {
        ...
        if (!alreadyRewardedTier && tierIndex != 0) {
          EscrowPayoutDetails storage payout = payoutIndexToPayouts[tierIndex];

          if (
            payout.amount > 0 && tokenBalances[payout.tokenId] >= payout.amount
          ) {
      ...
      bool alreadyRewardedTier = user.rewardGoalRewarded[escrow.rewardGoal];
      EscrowPayoutDetails storage payout = payoutIndexToPayouts[
        escrow.rewardGoal
      ];
      if (
        !alreadyRewardedTier && tokenBalances[payout.tokenId] >= payout.amount
      ) {
  ...
  function userWithdrawAll() external {
    ...
    uint256[] memory rewardedTokenAmounts = new uint256[](escrow.tokens.length);
  ...
  function creatorWithdrawToken(uint256 _tokenId, uint256 _amount) external {
    ...
    emit CreatorWithdraw(msg.sender, _tokenId, withdrawAmount, block.timestamp);
  }
  ...
  function setEscrowSettingsBasic(
    ...
    if (_condition == RewardCondition.SingleObjective) {
      ...
      if (!tiersAreActive) revert TiersMustBeActiveToUseTiersRewardCondition();
      require(
    ...
    emit ERC1155EscrowSettingsChanged(msg.sender, _condition, block.timestamp);
  }
  ...
  function runSetEscrowSettingsChecksBasic(
    ...
    if (msg.sender != creator) revert OnlyCreatorCanModifyEscrowSettings();
  ...
  function getEscrowTokenBalance(uint256 _tokenId)
    public
    view
    returns (uint256)
  {
  ...
  function getPayoutInfo(uint256 _rewardGoal)
    public
    view
    returns (uint256 tokenId, uint256 payoutAmount)
  {
    return (
  ...
  function getUserRewards(address _user)
    public
    view
    returns (UserTokenBalance[] memory)
  {
  ...
  function setDepositKey(bytes32 key, uint256 _depositEndDate) external {
    if (msg.sender != TEAM_ADDRESS) revert OnlyLoyaltyTeamCanSetDepositKey();
  ...
  function cancelProgramEscrow() external {
    if (msg.sender != creator) revert OnlyTeamCanCall();
    canceled = true;
  }
}
