...
abstract contract LoyaltyProgram is LoyaltySorting {
  ...
  function addTiers(
    ...
    if (_tierRewardsRequired[0] == 0) {
      ...
      tiers[0] = Tier({ name: bytes32("Default tier"), rewardsRequired: 0 });
      for (uint256 i = 0; i < _tierRewardsRequired.length; i++) {
        tiers[i + 1] = Tier({
          name: _tierNames[i],
          rewardsRequired: _tierRewardsRequired[i]
  ...
  function completeUserAuthorityObjective(uint256 _objectiveIndex) external {
    ...
    if (rewardType == RewardType.ERC20 && !tiersAreActive) {
      erc20EscrowContract.handleRewardsUnlock(msg.sender, _objectiveIndex);
    }

    if (rewardType == RewardType.ERC721 && !tiersAreActive) {
      erc721EscrowContract.handleRewardsUnlock(msg.sender, _objectiveIndex);
    }

    if (rewardType == RewardType.ERC1155 && !tiersAreActive) {
      erc1155EscrowContract.handleRewardsUnlock(msg.sender, _objectiveIndex);
    }
  ...
  function updateUserTierProgress(address _user) internal {
    ...
    if (rewardType == RewardType.ERC1155 || rewardType == RewardType.ERC20) {
  ...
  function getUserProgression(address _user)
    external
    view
    returns (uint256 rewardsEarned, uint256 currentTier)
  {
    return (users[_user].rewardsEarned, users[_user].currentTier);
  }

  function getUserObjectivesCompleteCount(address _user)
    external
    view
    returns (uint256)
  {
  ...
  function getUserCompletedObjectives(address _userAddress)
    external
    view
    returns (bool[] memory)
  {
  ...
  function setEscrowContract(address _contract, RewardType _rewardType)
    external
  {
    if (msg.sender != creator) revert OnlyCreatorCanCall();
