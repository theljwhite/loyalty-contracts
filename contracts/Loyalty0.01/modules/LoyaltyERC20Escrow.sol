...
//TODO - test if multiple separate deposits during deposit period accumulate balances correctly
//TODO - add frozen functionalty and test escrow complete/frozen, etc state
contract LoyaltyERC20Escrow {
  ...
  function depositBudget(uint256 _amount, address _token)
    external
    returns (uint256)
  {
  ...
  function handleTierRewardsUnlock(
    ...
    if (rewardCondition == RewardCondition.RewardPerTier) {
      for (uint256 i = 0; i < _passedTiers.length; i++) {
        ...
        if (!alreadyRewardedTier && tierIndex != 0 && escrowBalance >= amount) {
  ...
  function processUserProgressionRewards(address _user) private {
    ...
    if (rewardCondition == RewardCondition.AllObjectivesComplete) {
      ...
      if (
        rewardsEarned >= rewardGoal && !userAlreadyRewarded && escrowHasFunds
      ) {
        rewardGoalRewarded[_user][rewardGoal] = true;
        userBalance[_user] += payoutAmount;
        escrowBalance -= payoutAmount;
  ...
  function setEscrowSettingsBasic(
    ...
    if (_rewardCondition == RewardCondition.AllObjectivesComplete) {
      ...
      if (
        _rewardGoal == 0 || _rewardGoal > loyaltyProgram.totalPointsPossible()
      ) {
  ...
  function setEscrowSettingsAdvanced(
    ...
    if (_rewardCondition == RewardCondition.RewardPerObjective) {
      ...
      if (tierCount != _payouts.length) revert TiersAndPayoutLengthMismatch();
  ...
  function runSetEscrowSettingsChecks(RewardCondition _rewardCondition)
    private
    view
  {
  ...
  //TEMP
  function getAmountByPayoutIndex(uint256 _index)
    external
    view
    returns (uint256)
  {
